...
class KeychainMachineInventoryControllerIT extends BaseIntegrationTest {
    ...
    void setup() throws Exception {
        keychainMachineId = createKeychainMachine();
        productId = createProduct();
    }

    private String baseUrl() {
        return "/api/keychain-machines/" + keychainMachineId + "/inventory";
    }
    ...
    @Test
    void listInventory_keychainMachineNotFound_returns404() throws Exception {
        mockMvc.perform(get("/api/keychain-machines/" + UUID.randomUUID() + "/inventory"))
    ...
    void addInventory_valid_returns201() throws Exception {
        ...
        mockMvc.perform(post(baseUrl())
                ...
                .andExpect(jsonPath("$.quantity").value(10))
                .andExpect(jsonPath("$.keychainMachineId").value(keychainMachineId.toString()));
    }
    ...
    private UUID createKeychainMachine() throws Exception {
        KeychainMachineRequestDTO request = TestDataFactory.validKeychainMachineRequest();
        String response = mockMvc.perform(post("/api/keychain-machines")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(toJson(request)))
                .andExpect(status().isCreated())
                .andReturn()
    ...
    private UUID createInventory(UUID itemId) throws Exception {
        ...
        String response = mockMvc.perform(post(baseUrl())
                ...
                .getResponse()
                .getContentAsString();
        return fromJson(response, KeychainMachineInventoryResponseDTO.class).getId();
    }
}
