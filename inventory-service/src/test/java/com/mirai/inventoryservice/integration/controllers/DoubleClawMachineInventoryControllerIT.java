...
class DoubleClawMachineInventoryControllerIT extends BaseIntegrationTest {
    ...
    void setup() throws Exception {
        doubleClawMachineId = createDoubleClawMachine();
        productId = createProduct();
    }

    private String baseUrl() {
        return "/api/double-claw-machines/" + doubleClawMachineId + "/inventory";
    }
    ...
    @Test
    void listInventory_doubleClawMachineNotFound_returns404() throws Exception {
        mockMvc.perform(get("/api/double-claw-machines/" + UUID.randomUUID() + "/inventory"))
    ...
    void addInventory_valid_returns201() throws Exception {
        ...
        mockMvc.perform(post(baseUrl())
                ...
                .andExpect(jsonPath("$.quantity").value(10))
                .andExpect(jsonPath("$.doubleClawMachineId").value(doubleClawMachineId.toString()));
    }
    ...
    private UUID createDoubleClawMachine() throws Exception {
        DoubleClawMachineRequestDTO request = TestDataFactory.validDoubleClawMachineRequest();
        String response = mockMvc.perform(post("/api/double-claw-machines")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(toJson(request)))
                .andExpect(status().isCreated())
                .andReturn()
    ...
    private UUID createInventory(UUID itemId) throws Exception {
        ...
        String response = mockMvc.perform(post(baseUrl())
                ...
                .getResponse()
                .getContentAsString();
        return fromJson(response, DoubleClawMachineInventoryResponseDTO.class).getId();
    }
}
